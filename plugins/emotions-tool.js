...
cmd({
    pattern: "owner",
    desc: "Show owner name with cool style.",
    category: "info",
    react: "👑",
    filename: __filename
},
...
        const fullStyled = name.toUpperCase().split('').map(c => boxMap[c] || c);
        let msg = await conn.sendMessage(from, { text: "👑 *Owner:*" }, { quoted: m });

        let typed = '';
        for (let char of fullStyled) {
            typed += char;
            await conn.relayMessage(from, {
                protocolMessage: {
                    key: msg.key,
                    type: 14,
                    editedMessage: { conversation: `👑 *Owner:*\n${typed}` }
                }
            }, {});
